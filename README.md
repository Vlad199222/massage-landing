...
### Оптимізація зображень (WebP, PageSpeed)

Після додавання або зміни **PNG** у `public/images/` згенеруй поруч **WebP** (HTML уже підключає їх через `<picture>`):

```bash
npm run images:webp
```

Закоміть і `.png`, і `.webp`, щоб на Vercel віддавались стиснуті формати, а старі браузери лишались на PNG.
...
### Бот «не працює» / заявка не приходить

1. У браузері відкрий **`https://твій-сайт.vercel.app/api/booking`** (GET). Має бути JSON з `"telegramConfigured": true`. Якщо **false** — у Vercel не задані змінні або не зроблено **Redeploy** після їх додавання.
2. Якщо **404** на `/api/booking` — корінь проєкту в Vercel не той (наприклад, обрано лише `public/`) або репозиторій без папки **`api/`**. Має деплоїтися **весь** репозиторій з `api/booking.js`.
3. У **групі**: бот має бути **доданий у групу** з id `TELEGRAM_CHAT_ID`, не видалений; спробуй надіслати в групу команду `/start` боту або будь-яке повідомлення, потім ще раз заявку з сайту.
4. Якщо в формі з’являється текст з **«Деталь: …»** — це відповідь Telegram (наприклад *chat not found*, *bot was kicked*): перевір токен після `/revoke`, `chat_id` і учасників групи.
